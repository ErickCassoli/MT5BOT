...
from fxbot.core.utils import ema, atr, adx
...
def fetch_rates_chunked(symbol: str, timeframe: str, total: int, chunk: int = 20000) -> pd.DataFrame:
    ...
    while remaining > 0:
        ...
        if rates is None:
            ...
            if pos == 0:
                raise RuntimeError(f"copy_rates_from_pos falhou: {symbol}/{timeframe} -> {err}")
    ...
    if not got:
        raise RuntimeError(f"Sem dados retornados para {symbol}/{timeframe}. Último erro: {mt5.last_error()}")
...
def main():
    ap = argparse.ArgumentParser(description="Build dataset VKBP (VWAP + Keltner) com triple-barrier.")
    ...
    ap.add_argument("--chunk-size", type=int, default=20000, help="tamanho do lote p/ download de barras")
    ...
    for sym in args.symbols:
        ...
        df_e = fetch_rates_chunked(sym, args.tf_exec, args.bars_exec, args.chunk_size)
        df_r = fetch_rates_chunked(sym, args.tf_regime, args.bars_regime, args.chunk_size)
        ...
        for i in range(start, end, stride):
            ...
            uptrend  = (df_r_win["c"].iloc[-1] > ema200_h1.iloc[-1]) and (ema50_h1.iloc[-1] > ema200_h1.iloc[-1]) and (adx_h1.iloc[-1] >= args.adx_thr)
            downtrend = (df_r_win["c"].iloc[-1] < ema200_h1.iloc[-1]) and (ema50_h1.iloc[-1] < ema200_h1.iloc[-1]) and (adx_h1.iloc[-1] >= args.adx_thr)
            if not (uptrend or downtrend):
                continue

            dfw = df_e.iloc[:i+1].copy()
            mid, kup, klo, a = keltner(dfw, args.k_ema, args.k_atr, args.k_mult)
            a0 = float(a.iloc[-1])
            ...
            up0 = float(kup.iloc[-1]); lo0 = float(klo.iloc[-1])
            ...
            short_break = downtrend and (c0 < lo0) and ema20_ok and (c0 < vwap0)

            long_pull  = uptrend  and near_vwap and (rsi0 >= args.rsi_trig) and (c0 > vwap0) and ema20_ok
            short_pull = downtrend and near_vwap and (rsi0 <= (100-args.rsi_trig)) and (c0 < vwap0) and ema20_ok

            def add_row(side: str, trigger: str):
                y = label_triple_barrier(df_e, i, side, args.atr_sl, args.atr_tp, args.ahead)
                if y < 0:
                    return
                rows.append({
                    "symbol": sym,
            ...
            if long_break:  add_row("BUY",  "break")
            if short_break: add_row("SELL", "break")
            if long_pull:   add_row("BUY",  "pullback")
            if short_pull:  add_row("SELL", "pullback")

        print(f"[{sym}] amostras acumuladas: {len(rows)}")

    if not rows:
        raise SystemExit("Sem amostras geradas — ajuste parâmetros (k, near-vwap, adx, ahead/stride/barras).")
